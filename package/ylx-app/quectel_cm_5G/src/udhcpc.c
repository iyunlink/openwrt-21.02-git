...
static int ql_system(const char *udhcpc_cmd) {
    dbg_time("%s", udhcpc_cmd);
    return system(udhcpc_cmd);
}
...
static void ifc_set_state(const char *ifname, int state) {
    ...
    if (!access("/sbin/ip", X_OK)) {
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ip link set dev %s %s", ifname, state ? "up" : "down");
    } else {
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ifconfig %s %s", ifname, state ? "up" : "down");
    }
    ql_system(udhcpc_cmd);
}
...
void update_ipv4_address(const char *ifname, const char *ip, const char *gw, unsigned prefix)
{
    ...
    if (!access("/sbin/ip", X_OK)) {
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ip -%d address flush dev %s", 4, ifname);
        ql_system(udhcpc_cmd);

        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ip -%d address add %s/%u dev %s", 4, ip, prefix, ifname);
        ql_system(udhcpc_cmd);

        //ping6 www.qq.com
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ip -%d route add default via %s dev %s", 4, gw, ifname);
        ql_system(udhcpc_cmd);
    } else {
        unsigned n =  (0xFFFFFFFF >> (32 - prefix)) << (32 - prefix);
       // n = (n>>24) | (n>>8&0xff00) | (n<<8&0xff0000) | (n<<24);

        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ifconfig %s %s netmask %s", ifname, ip, ipv4Str(n));
        ql_system(udhcpc_cmd);

        //Resetting default routes
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "route del default dev %s", ifname);
        while(!system(udhcpc_cmd));

        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "route add default gw %s dev %s", gw, ifname);
        ql_system(udhcpc_cmd);
    }
}
...
void update_ipv6_address(const char *ifname, const char *ip, const char *gw, unsigned prefix) {
    ...
    (void)gw;
    if (!access("/sbin/ip", X_OK)) {
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ip -%d address flush dev %s", 6, ifname);
        ql_system(udhcpc_cmd);

        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ip -%d address add %s/%u dev %s", 6, ip, prefix, ifname);
        ql_system(udhcpc_cmd);

        //ping6 www.qq.com
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ip -%d route add default dev %s", 6, ifname);
        ql_system(udhcpc_cmd);
    } else {
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ifconfig %s %s/%d", ifname, ip, prefix);
        ql_system(udhcpc_cmd);

        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "route -A inet6 add default dev %s", ifname);
        ql_system(udhcpc_cmd);
    }
}
...
static int ql_openwrt_system(const char *cmd) {
    ...
    snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "%s 2>1 > /dev/null", cmd);
    
    for (i = 0; i < 15; i++) {
        dbg_time("%s", cmd);
        ret = system(udhcpc_cmd);
        if (!ret)
            break;
        sleep(1);
    }
...
static void ql_openwrt_setup_wan6(const char *ifname, const IPV6_T *ipv6) {
    ...
    if (openwrt_lan) {
        ...
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ip route del %s/%u dev %s", ipv6Str(Address), ipv6->PrefixLengthIPAddr, ifname);
        ql_openwrt_system(udhcpc_cmd);

        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ip route add %s/%u dev %s", ipv6Str(Address), ipv6->PrefixLengthIPAddr, openwrt_lan);
        ql_system(udhcpc_cmd);        
    }
#endif
}
...
/* Do DHCP using busybox tools */
    {
        ...
        pthread_attr_setdetachstate(&udhcpc_thread_attr, PTHREAD_CREATE_DETACHED);
            ...
            if (!access("/lib/netifd/dhcp.script", X_OK) && !access("/sbin/ifup", X_OK) && !access("/sbin/ifstatus", X_OK)) {
#if 1 //20210415 do not promot these message
                ...
                snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ifup $(uci show network | grep %s | grep -v 6 | awk -F . '{print $2}') && ifconfig %s up", ifname, ifname);
                ql_system(udhcpc_cmd);
#endif
            }
#endif
...
set_ipv6:
    if (profile->ipv6.Address[0] && profile->ipv6.PrefixLengthIPAddr) {
        ...
        */
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "route -A inet6 add default %s", ifname);
        ql_system(udhcpc_cmd);
...
//it seems when call netif_carrier_on(), and netcard 's IP is "0.0.0.0", will cause netif_queue_stopped()
    if (!access("/sbin/ip", X_OK))
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ip addr flush dev %s", ifname);
    else
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ifconfig %s 0.0.0.0", ifname);
    ql_system(udhcpc_cmd);
    ifc_set_state(ifname, 0);
